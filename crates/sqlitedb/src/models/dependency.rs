...
use crate::models::{module, dependency_type, dependency_module, gh_repository, gh_organization};
    ...
    conn.execute(
        format!("CREATE TABLE IF NOT EXISTS {0} (
            ...
            CONSTRAINT fk_dependency_type
                ...
                REFERENCES {1}(id)
                ON DELETE CASCADE
        )", &TABLE_NAME, &dependency_type::TABLE_NAME).as_str(),
        params![],
    )?;
    conn.execute(
        format!("CREATE UNIQUE INDEX IF NOT EXISTS uniq_dep_type_name ON {}(dependency_type_id, name)", &TABLE_NAME).as_str(),
        params![],
    )
}

fn query(conn: &Connection, extra_sql: &str, params: &[&dyn ToSql]) -> Result<Vec<Model>, rusqlite::Error> {
    let sql: String = format!("SELECT dep.id, dep.dependency_type_id, dt.name, dep.name \
    FROM {} as dep \
    INNER JOIN {} as dt \
    ON dt.id = dep.dependency_type_id \
    {}", &TABLE_NAME, &dependency_type::TABLE_NAME, &extra_sql);
...
pub fn get_module_external_dependency_names(conn: &Connection, module_id: &i64, dep_type: &str) -> Vec<String> {
    let mut stmt = conn.prepare(
        format!("SELECT dep.name \
        FROM {0} as dep \
        ...
        &TABLE_NAME, &dependency_module::TABLE_NAME, &dependency_type::TABLE_NAME, &module::TABLE_NAME).as_str(),
    ).unwrap();
...
pub fn get_module_dependency_info(conn: &Connection, module_id: &i64) -> Vec<DependencyModuleInfo> {
    let mut stmt = conn.prepare(
        format!("SELECT ghorg.name, ghrepo.name, dep.name \
        FROM {0} as dep \
        INNER JOIN {1} as dep_mod \
        ON dep_mod.dependency_id = dep.id \
        INNER JOIN {2} as dep_type \
        ...
        WHERE mod.id = ?1", 
        &TABLE_NAME, &dependency_module::TABLE_NAME, &dependency_type::TABLE_NAME, &module::TABLE_NAME, &gh_repository::TABLE_NAME, &gh_organization::TABLE_NAME).as_str(),
    ).unwrap();
    let deps_rows = stmt.query_map(
        params![&module_id], 
        |row| {
