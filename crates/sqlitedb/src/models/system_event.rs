...
fn query(
    ...
    let sql: String = format!(
        "SELECT se.message, se.date, se.event_type_id, syset.name \
    FROM {} as se \
    INNER JOIN {} as syset \
...
pub fn add(
    ...
    conn.execute(
        format!(
            "INSERT INTO {}(message, date, event_type_id) VALUES (?1, ?2, ?3)",
            &TABLE_NAME
        )
        .as_str(),
    ...
    Ok(Model {
        ...
        date,
        event_type_id: (system_event_type.id, system_event_type.name),
    })
}
...
pub fn register_new_gh_organization(
    ...
    let msg = format!(
        "New organization '<span class='org_name'>{}</span>' added",
        &org_name
    );
...
pub fn register_new_gh_repository(
    ...
    let msg = format!(
        "New repository '<span class='org_name'>{}</span>/<span class='repo_name'>{}</span>' added",
        &org_name, &repo_name
    );
