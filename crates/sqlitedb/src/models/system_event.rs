...
pub fn add(conn: &Connection, event_type_name: &str, message: &str) -> Result<Model, rusqlite::Error> {
    ...
    return Ok(Model { message: message.to_string(), date, event_type_id: (system_event_type.id, system_event_type.name) });
}
...
pub fn register_update_module(
    ...
    module_changes: &Vec<(&str, &str, &str)>,
    ...
    if !last_commit_partof.is_empty() {
        msg += format!(" - PR: <a class='git_pr' href='https://github.com/{}'>{}</a>", last_commit_partof.replace("#","/pull/"), &last_commit_partof).as_str();
    }
    msg += "</div>";
    add(&conn, "module", &msg)
}
