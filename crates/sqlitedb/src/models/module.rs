...
// Bitset DFS with memoization: each module's set of transitively reachable
// module ids is computed once and reused by every ancestor that also
// reaches it, instead of enumerating every (root, descendant) pair up
// front - a full pairwise-reachability recursive CTE over ~3-6k edges
// blows up to millions of rows on real data and hangs the page. `on_stack`
// breaks cycles: a back-edge into an in-progress ancestor is still counted
// (the edge itself) but not expanded further, matching how
// `dependency::get_full_dependency_info` already treats cycles.
fn reachable_bits(
    node: usize,
    adj: &[Vec<usize>],
    memo: &mut [Option<Vec<u64>>],
    on_stack: &mut [bool],
    words: usize,
) -> Vec<u64> {
    if let Some(bits) = &memo[node] {
        return bits.clone();
    }
    on_stack[node] = true;
    let mut bits = vec![0u64; words];
    for &child in &adj[node] {
        bits[child / 64] |= 1 << (child % 64);
        if !on_stack[child] {
            let child_bits = reachable_bits(child, adj, memo, on_stack, words);
            for (b, c) in bits.iter_mut().zip(&child_bits) {
                *b |= c;
            }
        }
    }
    on_stack[node] = false;
    memo[node] = Some(bits.clone());
    bits
}

/// For every module id appearing in `edges`, the size of its full
/// (transitive) reachable set.
fn full_dependency_counts(edges: &[ModuleEdgeRow]) -> HashMap<i64, i64> {
    ...
    for edge in edges {
        adj[index[&edge.from_id]].push(index[&edge.to_id]);
    }

    let mut memo: Vec<Option<Vec<u64>>> = vec![None; n];
    let mut on_stack = vec![false; n];
    index
        .iter()
        .map(|(&id, &idx)| {
            let bits = reachable_bits(idx, &adj, &mut memo, &mut on_stack, words);
            let count = bits.iter().map(|w| w.count_ones() as i64).sum();
            (id, count)
        })
        .collect()
}

/// The module with the largest full (transitive) Odoo dependency tree, at
/// the given Odoo version.
pub fn most_dependencies(
    conn: &mut SqliteConnection,
    version_odoo: &u8,
) -> Option<ModuleFunFactInfo> {
    let edges = diesel::sql_query(
        "SELECT dep_mod.module_id as from_id, mod_dep.id as to_id \
         FROM dependency_module as dep_mod \
         INNER JOIN dependency as dep ON dep.id = dep_mod.dependency_id \
         ...
         INNER JOIN module as mod_dep ON mod_dep.technical_name = dep.name \
             AND mod_dep.version_odoo = mod.version_odoo \
         WHERE dep_type.name = 'module' AND mod.version_odoo = ?",
    )
    .bind::<diesel::sql_types::Integer, _>(*version_odoo as i32)
    .load::<ModuleEdgeRow>(conn)
    .expect("DB error in module::most_dependencies (edges)");

    let (champion_id, value) = full_dependency_counts(&edges)
        .into_iter()
        .max_by_key(|&(_, count)| count)?;

    #[derive(QueryableByName)]
    struct ModuleOrgInfo {
        #[diesel(sql_type = diesel::sql_types::Text)]
        technical_name: String,
        #[diesel(sql_type = diesel::sql_types::Text)]
        organization: String,
    ...
    .bind::<diesel::sql_types::BigInt, _>(champion_id)
    .load::<ModuleOrgInfo>(conn)
    .expect("DB error in module::most_dependencies (champion info)")
    .into_iter()
    .next()
    .map(|info| ModuleFunFactInfo {
...
mod tests {
    use super::*;
    use crate::models::{dependency_module, dependency_type};
    ...
    fn make_module(conn: &mut SqliteConnection, tech_name: &str, git_repo: &str) -> Model {
        add(
            conn,
            &ManifestInfo {
    ...
    fn link_module_dep(conn: &mut SqliteConnection, from: &Model, to: &Model) {
    ...
    // root -> mid -> leaf: the champion must be root, counting the whole
    // tree (2), not just its direct dependency (1, which is what the old
    // one-hop COUNT(DISTINCT dep.id) query used to return).
    #[test]
    fn test_most_dependencies_counts_full_transitive_tree() {
        let mut conn = setup_db();
        let root = make_module(&mut conn, "root", "repo-root");
        let mid = make_module(&mut conn, "mid", "repo-mid");
        let leaf = make_module(&mut conn, "leaf", "repo-leaf");
        link_module_dep(&mut conn, &root, &mid);
        link_module_dep(&mut conn, &mid, &leaf);

        let champion = most_dependencies(&mut conn, &16).unwrap();
        assert_eq!(champion.technical_name, "root");
        assert_eq!(champion.value, 2);
    }

    // Cycle must not hang the recursive CTE. The closing edge (mid -> root)
    // is still a real edge reachable from root, so it counts (2), matching
    // how `dependency::get_full_dependency_info` already treats cycles.
    #[test]
    fn test_most_dependencies_handles_cycle_without_infinite_recursion() {
        let mut conn = setup_db();
        let root = make_module(&mut conn, "root", "repo-root");
        let mid = make_module(&mut conn, "mid", "repo-mid");
        link_module_dep(&mut conn, &root, &mid);
        link_module_dep(&mut conn, &mid, &root); // cycle back to root
