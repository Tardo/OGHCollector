...
use crate::models::{author, module, system_event};
...
pub fn create_table(conn: &Connection) -> Result<usize, rusqlite::Error> {
    ...
    conn.execute(
        format!(
            "CREATE UNIQUE INDEX IF NOT EXISTS uniq_module_author ON {}(module_id, author_id)",
            &TABLE_NAME
        )
        .as_str(),
        params![],
...
fn query(
    ...
    let sql: String = format!(
        "SELECT mod_au.id, mod_au.module_id, mod.technical_name, mod_au.author_id, au.name \
    FROM {} as mod_au \
    INNER JOIN {} as mod \
...
pub fn get_by_id(conn: &Connection, module_id: &i64, author_id: &i64) -> Option<Model> {
    let mod_authors = query(
        conn,
        "WHERE mod_au.module_id = ?1 AND mod_au.author_id = ?2 LIMIT 1",
        params![&module_id, &author_id],
    )
    .unwrap();
...
pub fn get_by_name(conn: &Connection, module_id: &i64, name: &str) -> Option<Model> {
    let mod_authors = query(
        conn,
        "WHERE mod_au.module_id = ?1 AND au.name = ?2 LIMIT 1",
...
pub fn get_top_names(conn: &Connection, limit: &u8) -> Vec<TopAuthorJSON> {
    let mut stmt = conn
        .prepare(
            format!(
                "SELECT author_id, count(*) as num_mods \
        FROM {} \
        GROUP BY author_id \
...
pub fn add(conn: &Connection, module_id: &i64, name: &str) -> Result<Model, rusqlite::Error> {
    ...
    if module_author_opt.is_none() {
        ...
        conn.execute(
            format!(
                "INSERT INTO {}(module_id, author_id) VALUES (?1, ?2)",
                &TABLE_NAME
            )
            .as_str(),
...
pub fn delete_by_module_id_author_id(
    ...
    conn.execute(
        format!(
            "DELETE FROM {} WHERE module_id = ?1 AND author_id = ?2",
            &TABLE_NAME
        )
        .as_str(),
        params![&module_id, &author_id],
