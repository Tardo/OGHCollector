...
use crate::models::{committer, module, system_event};
...
pub fn get_by_id(conn: &Connection, module_id: &i64, committer_id: &i64) -> Option<Model> {
    let mod_committers = query(
        conn,
        "WHERE mod_mant.module_id = ?1 AND mod_mant.committer_id = ?2 LIMIT 1",
        params![&module_id, &committer_id],
    )
    .unwrap();
...
pub fn get_by_name(conn: &Connection, module_id: &i64, name: &str) -> Option<Model> {
    let mod_committers = query(
        conn,
        "WHERE mod_com.module_id = ?1 AND com.name = ?2 LIMIT 1",
...
pub fn get_by_module_id(conn: &Connection, module_id: &i64) -> Vec<Model> {
    query(
        conn,
        "WHERE mod_com.module_id = ?1 LIMIT 1",
        params![&module_id],
    )
    .unwrap()
}
...
pub fn add(
    ...
    if module_committer_opt.is_none() {
        ...
        conn.execute(
            format!(
                "INSERT INTO {}(module_id, committer_id, commits) VALUES (?1, ?2, ?3)",
                &TABLE_NAME
            )
            .as_str(),
    ...
    conn.execute(
        format!(
            "UPDATE {} SET commits = ?3 WHERE module_id = ?1 AND committer_id = ?2",
            &TABLE_NAME
        )
        .as_str(),
        params![
            &module_committer.module_id.0,
            &module_committer.committer_id.0,
            &commits
        ],
    )?;
    Ok(Model {
